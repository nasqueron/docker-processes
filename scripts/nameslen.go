// Names Len is a small program that fetches the list
// of containers random names and count the max len.
...
//  -------------------------------------------------------------
//  Vars from github.com/docker/docker/pkg/namesgenerator package
//  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
func GetLongestContainerName() string {
	return stringutilities.GetLongestWord(left[:]) + "_" + stringutilities.GetLongestWord(right[:])
}
...
func main () {
	name := GetLongestContainerName()
	fmt.Printf("Longest container size: %d\n", len(name))
	fmt.Printf("Longest container name: %s\n", name)
}
